...
export const listExpenses = () => async (dispatch) => {
    ...
    try {
        dispatch({ type: LIST_EXPENSES_REQUEST })

        const { data } = await axiosInstance.get(`${process.env.REACT_APP_API_URL}/expenses`);
...
export const addExpense = (expenseData) => async (dispatch) => {
    ...
    try {
        ...
        const { data } = await axiosInstance.post(`${process.env.REACT_APP_API_URL}/expenses`, expenseData, config);
